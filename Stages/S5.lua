Set5={
	name = GetString( MAHELPER_ARENA_5 ),
	nR = 5,
	rounds = {			
		--first round
		[1] = {
			...
			Waves = {
				...
				[2] = {
					...
					Mobs = {
						...
						[2] = {posx =165958,posy =49955,posz =100564,X=0.51183432340622,Y=0.5088757276535, texture = "MAHelper/Icons/Signal2.dds", c = 1,},
						[3] = {posx =166476,posy =49975,posz =99051,X=0.51923078298569,Y=0.48668637871742, texture = "MAHelper/Icons/Signal2.dds", c = 1,},
					},
				},
				[3] = {
					nMOB = 4,
...
function Set5.OnCheckingEffect( eventCode,  changeType,  effectSlot,  effectName,  unitTag,  beginTime,  endTime,  stackCount,  iconName,  buffType,  effectType,  abilityType,  statusEffectType,  unitName,  unitId,  abilityId,  sourceUnitType)
	if effectName == GetString(MAHELPER_PORTAL_SPAWN) and NumK == 0 and token then
		token = false 
		zo_callLater(function() token = true end, 7000)
	end
	if effectName == GetString(MAHELPER_PORTAL_SPAWN) and unitName == GetString(MAHELPER_R5_SM)  then
...
function Set5.OnUnitKilled(eventCode,   result,  isError,  abilityName,  abilityGraphic,   abilityActionSlotType,  sourceName,   sourceType,   targetName,   targetType,  hitValue,   powerType,   damageType,  log,  sourceUnitId,  targetUnitId,  abilityId,  overflow)
	d(targetName)
	if targetName ==  GetString(MAHELPER_R5_DW) or targetName ==  GetString(MAHELPER_R5_G_BOR) then
		do return end
	end
	if CurrStageArena == 5 then
		if targetName ~= GetString(MAHELPER_R5_BN) then
			do return end
		else
			MAHelper.RemoveIcons()
			do return end
